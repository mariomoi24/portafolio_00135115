...
void eliminarLibro(){
    ...
    while(p != NULL && (p->dato).genero != unGenero)
        q = p;
        p = p->sig;
...
void  generoaBuscar(){
 char unGenero = 0;
    ...
    while(s != NULL && (s->dato).genero != unGenero)
        s = s->sig;
...
void libroEspanol(){
    ...
    while(s != NULL && (s->dato).espanol != enEspanol)
        s = s->sig;
...
int main(){
    ...
    do{
        ...
        cout << "Menu: \n\t1) Agregar Libro\n\t2) Ver Libros"
            << "\n\t3) Eliminar libro\n\t4) Buscar Libro"
               <<"/n/t5. Buscar genero"
            << "\n\t6) Buscar libro en Espanol\n\t 7. Salir Opcion elegida: ";
        cin >> opcion;
        switch(opcion){
            ...
            case 5:cout<<"El genero si se encuentra"<<endl;
             generoaBuscar();
			
				break;
				
			case 6:cout<<"El libro esta en Espanol "<<endl;
			libroEspanol();
			break;
